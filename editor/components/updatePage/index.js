...
export default {
    ...
    methods: {
        async initPage() {
            ...
            var article = await fetch(`/${this.$route.params.pageFolder}/${utilities.deQuote(utilities.encodeTitle(this.updatedPage.title))}.html`)
            article = await article.text()
            article = utilities.extractArticle(article)

            this.updatedPage.article = article
